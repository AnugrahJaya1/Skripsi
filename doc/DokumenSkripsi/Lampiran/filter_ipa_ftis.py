...
mpm = pd.DataFrame(columns=["id_mata_pelajaran_mahasiswa","id_user","id_mata_pelajaran"]);


def generateCSV(mhs, nilai, mpm, count):
    size= data.shape[0]
    batas = int(size/(4*count))
    
    id_user=1529  #pake counter
    id_nilai=3609 #pake counter
    id_mpm=3609 #pake counter
    ...
    for idx in range(batas):
        ...
        for i in range(0,count):
            ...
            nilai = nilai.append({"id_nilai":id_nilai, "id_mata_pelajaran":id_mp, "id_user":id_user,
                                ...
                                ignore_index=True)
            #masukin ke mpm
            #mpm = mpm.append({"id_mata_pelajaran_mahasiswa":id_mpm, "id_user":id_user, "id_mata_pelajaran":id_mp},
            #                 ignore_index=True)
    ...
    return mhs, nilai, mpm
    #mhs = mhs.to_excel(r"C:\Users\anugrahjaya1\Downloads\Data\new_akun_mhs.xlsx", index=None, header=True)

    #return mhs
rest = generateCSV(mhs, nilai, mpm, 2)    
...
#mpm = rest[2]
#mpm.to_csv(r"C:\Users\anugrahjaya1\Downloads\Data\7 FTIS\new_ftis_mpm2.csv", index=None, header=True)